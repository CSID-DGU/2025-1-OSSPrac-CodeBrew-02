...
def index():
    ...
    return render_template('index.html', result=result)  # 결과 데이터를 index.html로 전달
...
def result():
    if request.method == 'POST':
        ...
        # 학생 정보 리스트 생성
        zipped_result = list(zip(names, roles, student_numbers, genders, majors, languages, masked_emails, masked_phones, mbtis, selected_pictures))
        session['result'] = zipped_result  # 세션에 결과 저장

        return render_template('result.html', result=zipped_result)  # 결과 데이터를 result.html로 전달
